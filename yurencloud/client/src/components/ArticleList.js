        ...
        <Content style={{ background: '#fff', padding: 24, margin: 0 }}>
          ...
          <div className={style.inlineElement}>
            <ButtonGroup >
              ...
              >点赞 <Icon type={good === 'desc' ? 'down' : 'up'} /></Button>
              <Button
                onClick={() => {
          ...
          <Table dataSource={pageInfo.list} rowKey="id" pagination={pagination}>
            <Column
              ...
              render={(text, record) => (
                <div>
                  <span>
                    {record.top === 0 ? '' : <Icon type="pushpin-o" style={{ color: 'red' }} />}
                  </span>
                  <span>
                    {record.recommend === 0 ? '' : <Icon type="star-o" style={{ color: 'red' }} />}
                  </span>
            ...
            <Column
              ...
              render={(text, record) => (
                <span>
                  {dateformate(record.createdAt, 'mm月dd日')}
                </span>
              )}
            />
            <Column
              ...
              key="good"
              render={(text, record) => (
                <span> <Icon type="like-o" /> {record.good}</span>
              )}
            />
            <Column
              title="浏览"
              key="view"
              render={(text, record) => (
                <span> <Icon type="eye-o" /> {record.view}</span>
              )}
            />
            <Column
              title="操作"
