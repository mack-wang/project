...
public class InvoiceRequestSpec {
    public static Specification<InvoiceRequest> getSpec(
            final User user,
            final Integer  result
    ) {//这里的参数也可以是多个，名字也可以随便取，只要是供下面条件判断使用
        return new Specification<InvoiceRequest>() {
            @Override
            public Predicate toPredicate(Root<InvoiceRequest> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
