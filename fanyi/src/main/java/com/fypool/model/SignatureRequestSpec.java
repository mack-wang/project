...
public class SignatureRequestSpec {
    public static Specification<SignatureRequest> getSpec(
            final User user,
            final Integer  result
    ) {//这里的参数也可以是多个，名字也可以随便取，只要是供下面条件判断使用
        return new Specification<SignatureRequest>() {
            @Override
            public Predicate toPredicate(Root<SignatureRequest> root, CriteriaQuery<?> query, CriteriaBuilder criteriaBuilder) {
                ...
                if (result != null) {
                    Predicate p2 = criteriaBuilder.equal(root.get("result"), result);
                    if (p1 != null) {
                        p1 = criteriaBuilder.and(p1, p2);
                    } else {
                        p1 = p2;
